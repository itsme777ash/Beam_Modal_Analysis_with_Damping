E = [121e9,210e9]; q = 0; P = 100; M = 0; density = [8900,7850];b = [0.05,0.05]; h = [0.01,0.01]; I =b.*(h.^3)/12;
damp_ratio = 0.1;
...
for element = 1:1:Element_no
    ...
    k(:,:,element) = Beam_stiff_matrix(E(elem_s_no),I(elem_s_no),[(element-1)*elem_len,element*elem_len]);
    Mass(:,:,element) = Beam_Mass_matrix(b(elem_s_no)*h(elem_s_no),density(elem_s_no),elem_len);
...
plot(linspace(5,100,96),Normalized_amp);
